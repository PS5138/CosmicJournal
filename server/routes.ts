...
async function extractMediaFromApodPage(date: string) {
  try {
    ...
    // Extract video source (first try <source src="...mp4">)
    ...
    // If no <source>, try to extract <iframe src="...">
    ...
    if (iframeMatch) {
      ...
      return {
        media_type: "video",
        url: iframeUrl.startsWith("http")
          ? iframeUrl
          : `https://apod.nasa.gov/apod/${iframeUrl}`,
        extracted: true,
      };
    }
    ...
    const imageMatch = html.match(
      /<a\s+href="([^"]*\.(?:jpg|jpeg|png|gif)[^"]*)"[^>]*><img/i,
    );
    if (imageMatch) {
  ...
  app.get("/api/apod", async (req, res) => {
    try {
      ...
      if (needsExtraction && date && typeof date === "string") {
        console.log(
          `Attempting to extract media for ${date} from NASA APOD page...`,
        );
        const extractedMedia = await extractMediaFromApodPage(date);

        if (extractedMedia) {
          console.log(
            `Successfully extracted ${extractedMedia.media_type} URL:`,
            extractedMedia.url,
          );
